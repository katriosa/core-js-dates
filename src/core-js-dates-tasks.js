...
function getNextFriday(date) {
  ...
  if (dayOfWeekIndex === 5) {
    ...
    addDays = dayOfWeekIndex < 5 ? 5 - dayOfWeekIndex : dayOfWeekIndex;
  }
  const nextFriday = new Date(dateObj);
  nextFriday.setDate(dateObj.getDate() + addDays);
  return nextFriday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const daysOnPeriod = Math.ceil(ms / msInDay) + 1;
  return daysOnPeriod;
}
...
function formatDate(date) {
  ...
  const hoursIn24Format = dateObj.getUTCHours();
  const ampm = hoursIn24Format >= 12 ? 'PM' : 'AM';

  const hoursIn12Format =
    hoursIn24Format > 12 ? hoursIn24Format - 12 : hoursIn24Format;

  const minutes = dateObj.getMinutes().toString().padStart(2, '0');
  const seconds = dateObj.getSeconds().toString().padStart(2, '0');

  return `${month}/${day}/${year}, ${hoursIn12Format}:${minutes}:${seconds} ${ampm}`;
}
